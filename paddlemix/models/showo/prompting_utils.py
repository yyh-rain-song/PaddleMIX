...
class UniversalPrompting:
    ...
    def __init__(self, text_tokenizer, special_tokens=('<|soi|>', '<|eoi|>',
        '<|sov|>', '<|eov|>', '<|t2i|>', '<|mmu|>', '<|t2v|>', '<|v2v|>',
        '<|lvg|>'), max_text_len=8000, max_seq_len=377, ignore_id=-100,
        ...
        self.sptids_dict = {token: paddle.to_tensor(data=self.
            text_tokenizer.convert_tokens_to_ids([token])) for token in
            special_tokens}
        self.sptids_dict['<|sot|>'] = paddle.to_tensor(data=[self.
            text_tokenizer.bos_token_id])
        self.sptids_dict['<|eot|>'] = paddle.to_tensor(data=[self.
            text_tokenizer.eos_token_id])
        self.sptids_dict['<|pad|>'] = paddle.to_tensor(data=[self.
    ...
    def t2i_prompt(self, text_ids, image_ids, labels):
        ...
        for i in range(len(text_ids)):
            ...
            temp_ids = [int(self.sptids_dict['<|t2i|>'])] + text_ids[i] + [self
                .text_tokenizer.eos_token_id]
            if probs[i] < self.cond_dropout_prob:
                temp_ids = [int(self.sptids_dict['<|t2i|>']), self.
                    text_tokenizer.bos_token_id, self.text_tokenizer.
                    eos_token_id]
            if self.max_text_len >= len(temp_ids):
                temp_ids = [self.pad_id] * (self.max_text_len - len(temp_ids)
                    ) + temp_ids
                temp_masks = [0] * (self.max_text_len - len(temp_ids)) + [1
                    ] * (len(temp_ids) + tuple(image_ids.shape)[-1] + 3)
            else:
                temp_ids = temp_ids[:self.max_text_len - 1] + [self.
                    text_tokenizer.eos_token_id]
                temp_masks = [1] * (len(temp_ids) + tuple(image_ids.shape)[
                    -1] + 3)
            temp_label_ids = paddle.concat(x=[paddle.to_tensor(data=
                temp_ids).to(device), self.sptids_dict['<|soi|>'].to(device
                ), labels[i], self.sptids_dict['<|eoi|>'].to(device)], axis=0)
            temp_label_ids = paddle.where(condition=temp_label_ids == self.
                pad_id, x=self.ignore_id, y=temp_label_ids)
            temp_ids = paddle.concat(x=[paddle.to_tensor(data=temp_ids).to(
                device), self.sptids_dict['<|soi|>'].to(device), image_ids[
                i], self.sptids_dict['<|eoi|>'].to(device)], axis=0)
        ...
        return paddle.concat(x=sequence_ids, axis=0), paddle.concat(x=
            attention_masks, axis=0), paddle.concat(x=label_ids, axis=0)
    ...
    def lm_prompt(self, text_ids, max_seq_len):
        ...
        for i in range(len(text_ids)):
            ...
            if max_seq_len >= len(temp_ids):
                temp_labels_ids = temp_ids + [self.ignore_id] * (max_seq_len -
                    len(temp_ids))
                temp_ids = temp_ids + [self.pad_id] * (max_seq_len - len(
                    temp_ids))
                temp_masks = [1] * len(temp_ids) + [0] * (max_seq_len - len
                    (temp_ids))
            else:
    ...
    def mmu_prompt(self, image_ids, text_ids):
        ...
        for i in range(len(text_ids)):
            ...
            if max_text_len >= len(temp_ids):
                temp_ids = temp_ids + [self.pad_id] * (max_text_len - len(
                    temp_ids))
                temp_masks = [1] * (len(temp_ids) + tuple(image_ids.shape)[
                    -1] + 3) + [0] * (max_text_len - len(temp_ids))
            else:
                temp_ids = temp_ids[:max_text_len - 1] + [self.
                    text_tokenizer.eos_token_id]
                temp_masks = [1] * (len(temp_ids) + tuple(image_ids.shape)[
                    -1] + 3)
            temp_label_ids = paddle.concat(x=[paddle.to_tensor(data=[self.
                ignore_id]).to(device), paddle.to_tensor(data=[self.
                ignore_id]).to(device), paddle.ones_like(x=image_ids[i]) *
                self.ignore_id, paddle.to_tensor(data=[self.ignore_id]).to(
                device), paddle.to_tensor(data=temp_ids).to(device)], axis=0)
            temp_label_ids = paddle.where(condition=temp_label_ids == self.
                pad_id, x=self.ignore_id, y=temp_label_ids)
            temp_ids = paddle.concat(x=[self.sptids_dict['<|mmu|>'].to(
                device), self.sptids_dict['<|soi|>'].to(device), image_ids[
                i], self.sptids_dict['<|eoi|>'].to(device), paddle.
                to_tensor(data=temp_ids).to(device)], axis=0)
            temp_masks = paddle.to_tensor(data=temp_masks).to(device)
            sequence_ids.append(temp_ids.unsqueeze(axis=0))
            attention_masks.append(temp_masks.unsqueeze(axis=0))
            label_ids.append(temp_label_ids.unsqueeze(axis=0))
    ...
    def t2v_prompt(self, text_ids, image_ids, labels):
        ...
        for i in range(len(text_ids)):
            if len(text_ids[i]) == 0:
                ...
                text_ids[i] = [self.text_tokenizer.bos_token_id] + text_ids[i]
            temp_ids = [int(self.sptids_dict['<|t2v|>'])] + text_ids[i] + [self
                .text_tokenizer.eos_token_id]
            if probs[i] < self.cond_dropout_prob:
                temp_ids = [int(self.sptids_dict['<|t2v|>']), self.
                    text_tokenizer.bos_token_id, self.text_tokenizer.
                    eos_token_id]
            if self.max_text_len >= len(temp_ids):
                temp_ids = [self.pad_id] * (self.max_text_len - len(temp_ids)
                ...
                temp_masks = [1] * (len(temp_ids) + tuple(image_ids.shape)[
                    -1] + 3)
            temp_label_ids = paddle.concat(x=[paddle.to_tensor(data=
                temp_ids).to(device), self.sptids_dict['<|sov|>'].to(device
                ), labels[i], self.sptids_dict['<|eov|>'].to(device)], axis=0)
            temp_label_ids = paddle.where(condition=temp_label_ids == self.
                pad_id, x=self.ignore_id, y=temp_label_ids)
            temp_ids = paddle.concat(x=[paddle.to_tensor(data=temp_ids).to(
                device), self.sptids_dict['<|sov|>'].to(device), image_ids[
                i], self.sptids_dict['<|eov|>'].to(device)], axis=0)
            temp_masks = paddle.to_tensor(data=temp_masks).to(device)
            sequence_ids.append(temp_ids.unsqueeze(axis=0))
            attention_masks.append(temp_masks.unsqueeze(axis=0))
            label_ids.append(temp_label_ids.unsqueeze(axis=0))
    ...
    def __call__(self, input, task, padding=True, config=None):
        ...
        if task == 't2i':
            ...
            text_ids = self.text_tokenizer(input[0])['input_ids']
            image_ids = input[1]
            sequence_ids_with_masks = self.t2v_prompt(text_ids, image_ids,
                input[2])
        elif task == 't2i_plus_lm':
            text_ids = self.text_tokenizer(input[0])['input_ids']
            image_ids = input[1]
            sequence_ids_with_masks = self.t2i_prompt(text_ids[:config.
                training.batch_size], image_ids, input[2])
            sequence_ids_with_masks_lm = self.lm_prompt(text_ids[config.
                training.batch_size:], input[3])
            ...
            text_ids = self.text_tokenizer(input[0])['input_ids']
            image_ids = input[1]
            sequence_ids_with_masks = self.lvg_prompt(text_ids, image_ids,
                input[2])
        elif task == 'lvg_gen':
            text_ids = self.text_tokenizer(input[0])['input_ids']
            image_ids = input[1]
...
def create_attention_mask_predict_next(sequence, pad_id=128256, soi_id=128257, eoi_id=128258, rm_pad_in_image=False,
    ...
    causal_mask = paddle.tril(x=paddle.ones(shape=(L, L), dtype='bool'))
...
def create_attention_mask_lvg(sequence, pad_id=128256, soi_id=128257, eoi_id=128258, return_inverse_mask=True):
    ...
    # Masks to identify different types of tokens
    is_padding = sequence == pad_id
    mask_text_image_bi = paddle.tril(x=paddle.ones(shape=[N, L, L]), diagonal=0
        ).to(sequence.place)
    sid_img = paddle.where(sequence == soi_id)[1].reshape(tuple(
        mask_text_image_bi.shape)[0], -1)[:, 0]
    sid_img_for_bi = paddle.where(sequence == soi_id)[1].reshape(tuple(
        mask_text_image_bi.shape)[0], -1)
    eid_img_for_bi = paddle.where(sequence == eoi_id)[1].reshape(tuple(
        mask_text_image_bi.shape)[0], -1)
    for i in range(N):
        id_padding = paddle.where(is_padding[i] == True)
        mask_text_image_bi[i][sid_img[i]:, id_padding[0]] = 0
        for j in range(tuple(sid_img_for_bi.shape)[-1]):
            mask_text_image_bi[i][sid_img_for_bi[i, j]:eid_img_for_bi[i, j] +
                1, sid_img_for_bi[i, j]:eid_img_for_bi[i, j] + 1] = 1
    if return_inverse_mask:
...
def create_attention_mask_lvg_v2(sequence, pad_id=128256, soi_id=128257,
    eoi_id=128258, sot_id=1000, eot_id=1001, return_inverse_mask=True):
    N, L = tuple(sequence.shape)
    is_padding = sequence == pad_id
    is_text = paddle.where(condition=sequence < pad_id, x=True, y=False)
    mask_text_image_bi = paddle.tril(x=paddle.ones(shape=[N, L, L]), diagonal=0
        ).to(sequence.place).astype(dtype='int32')
    sid_text_for_bi = paddle.where(sequence == sot_id)[1].reshape(tuple(
        mask_text_image_bi.shape)[0], -1)
    eid_text_for_bi = paddle.where(sequence == eot_id)[1].reshape(tuple(
        mask_text_image_bi.shape)[0], -1)
    if sot_id == eot_id:
        ...
        select_idx = [(i + 1) for i in range(0, tuple(eid_text_for_bi.shape
            )[1], 2)]
    ...
    for i in range(N):
        ...
        for j in range(tuple(sid_text_for_bi.shape)[-1]):
            all_zeros[i][is_text[i], sid_text_for_bi[i, j]:eid_text_for_bi[
                i, j] + 1] = 1
            all_zeros[i][~is_text[i], sid_text_for_bi[i, j]:eid_text_for_bi
                [i, j] + 1] = 1
        for j in range(tuple(sid_img_for_bi.shape)[-1]):
            all_zeros[i][~is_text[i], sid_img_for_bi[i, j]:eid_img_for_bi[i,
                j] + 1] = 1
...
def create_attention_mask_for_mmu(sequence, eoi_id=128258,
    return_inverse_mask=True):
    N, L = tuple(sequence.shape)
    causal_mask = paddle.tril(x=paddle.ones(shape=(N, 1, L, L), dtype='bool')
        ).to(sequence.place)
...
def create_attention_mask_for_mmu_vit(sequence, return_inverse_mask=True,
    system_prompt_len=0):
