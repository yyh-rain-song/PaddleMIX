...
# >>>>>>logger = diffusers.utils.logging.get_logger(__name__)
# _REGEX_SHARD = re.compile('(.*?)-\\d{5}-of-\\d{5}')
# >>>>>>if diffusers.utils.is_torch_version('>=', '1.9.0'):
#     _LOW_CPU_MEM_USAGE_DEFAULT = True
# else:
#     _LOW_CPU_MEM_USAGE_DEFAULT = False
# >>>>>>if diffusers.utils.is_accelerate_available():
#     pass


def get_parameter_device(parameter: paddle.nn.Layer) ->(paddle.CPUPlace,
    paddle.CUDAPlace, str):
    try:
        parameters_and_buffers = itertools.chain(parameter.parameters(),
            parameter.buffers())
        return next(parameters_and_buffers).place
    except StopIteration:

        def find_tensor_attributes(module: paddle.nn.Layer) ->List[Tuple[
            str, paddle.Tensor]]:
            tuples = [(k, v) for k, v in module.__dict__.items() if paddle.
                is_tensor(x=v)]
            return tuples
        gen = parameter._named_members(get_members_fn=find_tensor_attributes)
        first_tuple = next(gen)
        return first_tuple[1].place


def get_parameter_dtype(parameter: paddle.nn.Layer) ->paddle.dtype:
    try:
...
class ModelMixin(paddle.nn.Layer, diffusers.utils.hub_utils.PushToHubMixin):
    """
    ...
    def __getattr__(self, name: str) ->Any:
        ...
        if is_in_config and not is_attribute:
            ...
            diffusers.utils.deprecate('direct config name access', '1.0.0',
                deprecation_message, standard_warn=False, stacklevel=3)
    ...
    def is_gradient_checkpointing(self) ->bool:
        ...
        return any(hasattr(m, 'gradient_checkpointing') and m.
            gradient_checkpointing for m in self.sublayers())
    ...
    def set_use_npu_flash_attention(self, valid: bool) ->None:
        ...
        def fn_recursive_set_npu_flash_attention(module: paddle.nn.Layer):
        ...
        for module in self.children():
            if isinstance(module, paddle.nn.Layer):
    ...
    def save_pretrained(self, save_directory: Union[str, os.PathLike],
        ...
        max_shard_size: Union[int, str]='10GB', push_to_hub: bool=False, **
        kwargs):
        """
        ...
        weights_name = (SAFETENSORS_WEIGHTS_NAME if safe_serialization else
            WEIGHTS_NAME)
>>>>>>        weights_name = diffusers.utils._add_variant(weights_name, variant)
        weight_name_split = weights_name.split('.')
        if len(weight_name_split) in [2, 3]:
            weights_name_pattern = weight_name_split[0
                ] + '{suffix}.' + '.'.join(weight_name_split[1:])
        else:
        ...
        if push_to_hub:
            ...
            repo_id = kwargs.pop('repo_id', save_directory.split(os.path.
                sep)[-1])
        ...
        state_dict_split = split_torch_state_dict_into_shards(state_dict,
            max_shard_size=max_shard_size, filename_pattern=
        ...
        if is_main_process:
            for filename in os.listdir(save_directory):
                ...
                if not os.path.isfile(full_filename):
                    continue
                weights_without_ext = weights_name_pattern.replace('.bin', ''
                    ).replace('.safetensors', '')
                weights_without_ext = weights_without_ext.replace('{suffix}',
                    '')
                filename_without_ext = filename.replace('.bin', '').replace(
                    '.safetensors', '')
                if filename.startswith(weights_without_ext
                    ) and _REGEX_SHARD.fullmatch(filename_without_ext
                    ) is not None:
        ...
        for filename, tensors in state_dict_split.filename_to_tensors.items():
            ...
            if safe_serialization:
                safetensors.torch.save_file(shard, filepath, metadata={
                    'format': 'pt'})
            else:
                paddle.save(obj=shard, path=filepath)
        if state_dict_split.is_sharded:
            index = {'metadata': state_dict_split.metadata, 'weight_map':
                state_dict_split.tensor_to_filename}
            save_index_file = (SAFE_WEIGHTS_INDEX_NAME if
                safe_serialization else WEIGHTS_INDEX_NAME)
>>>>>>            save_index_file = os.path.join(save_directory, diffusers.utils.
                _add_variant(save_index_file, variant))
            with open(save_index_file, 'w', encoding='utf-8') as f:
                content = json.dumps(index, indent=2, sort_keys=True) + '\n'
                f.write(content)
            logger.info(
                f'The model is bigger than the maximum size per checkpoint ({max_shard_size}) and is going to be split in {len(state_dict_split.filename_to_tensors)} checkpoint shards. You can find where each parameters has been saved in the index located at {save_index_file}.'
                )
        else:
            path_to_weights = os.path.join(save_directory, weights_name)
            logger.info(f'Model weights saved in {path_to_weights}')
        if push_to_hub:
>>>>>>            model_card = diffusers.utils.hub_utils.load_or_create_model_card(
                repo_id, token=token)
>>>>>>            model_card = diffusers.utils.hub_utils.populate_model_card(
                model_card)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[
        ...
        if use_safetensors is None:
            ...
            allow_pickle = True
>>>>>>        if low_cpu_mem_usage and not diffusers.utils.is_accelerate_available():
...
>>>>>>        if (device_map is not None and not diffusers.utils.
...
>>>>>>        if device_map is not None and not diffusers.utils.is_torch_version('>='
            , '1.9.0'):
            raise NotImplementedError(
                'Loading and dispatching requires torch >= 1.9.0. Please either update your PyTorch version or set `device_map=None`.'
                )
>>>>>>        if low_cpu_mem_usage is True and not diffusers.utils.is_torch_version(
            '>=', '1.9.0'):
            raise NotImplementedError(
        ...
        if isinstance(device_map, (paddle.CPUPlace, paddle.CUDAPlace, str)):
            device_map = {'': device_map}
        elif isinstance(device_map, str) and device_map not in ['auto',
            'balanced', 'balanced_low_0', 'sequential']:
            try:
                device_map = {'': str(device_map).replace('cuda', 'gpu')}
            except RuntimeError:
                raise ValueError(
...
>>>>>>        user_agent = {'diffusers': diffusers.__version__, 'file_type':
            'model', 'framework': 'pytorch'}
        config, unused_kwargs, commit_hash = cls.load_config(config_path,
            cache_dir=cache_dir, return_unused_kwargs=True,
            return_commit_hash=True, force_download=force_download, proxies
...
>>>>>>        index_file = diffusers.models.model_loading_utils._fetch_index_file(
            is_local=is_local, pretrained_model_name_or_path=
            pretrained_model_name_or_path, subfolder=subfolder or '',
            use_safetensors=use_safetensors, cache_dir=cache_dir, variant=
            variant, force_download=force_download, proxies=proxies,
            local_files_only=local_files_only, token=token, revision=
            revision, user_agent=user_agent, commit_hash=commit_hash)
        if index_file is not None and index_file.is_file():
            is_sharded = True
        if is_sharded and from_flax:
            raise ValueError(
                'Loading of sharded checkpoints is not supported when `from_flax=True`.'
                )
        model_file = None
        if from_flax:
>>>>>>            model_file = diffusers.utils._get_model_file(
                pretrained_model_name_or_path, weights_name=
                FLAX_WEIGHTS_NAME, cache_dir=cache_dir, force_download=
            ...
            if is_sharded:
>>>>>>                sharded_ckpt_cached_folder, sharded_metadata = (diffusers.
                    utils._get_checkpoint_shard_files(
                    pretrained_model_name_or_path, index_file, cache_dir=
                    cache_dir, proxies=proxies, local_files_only=
                    local_files_only, token=token, user_agent=user_agent,
                    revision=revision, subfolder=subfolder or ''))
            elif use_safetensors and not is_sharded:
                try:
>>>>>>                    model_file = diffusers.utils._get_model_file(
                        pretrained_model_name_or_path, weights_name=
>>>>>>                        diffusers.utils._add_variant(
                        SAFETENSORS_WEIGHTS_NAME, variant), cache_dir=
                        cache_dir, force_download=force_download, proxies=
                        proxies, local_files_only=local_files_only, token=
                        token, revision=revision, subfolder=subfolder,
...
>>>>>>                model_file = diffusers.utils._get_model_file(
>>>>>>                    pretrained_model_name_or_path, weights_name=diffusers.
                    utils._add_variant(WEIGHTS_NAME, variant), cache_dir=
                    cache_dir, force_download=force_download, proxies=
                    proxies, local_files_only=local_files_only, token=token,
                    revision=revision, subfolder=subfolder, user_agent=
                    user_agent, commit_hash=commit_hash)
            if low_cpu_mem_usage:
>>>>>>                with accelerate.init_empty_weights():
                    model = cls.from_config(config, **unused_kwargs)
                if device_map is None and not is_sharded:
                    param_device = 'cpu'
>>>>>>                    state_dict = (diffusers.models.model_loading_utils.
                        load_state_dict(model_file, variant=variant))
...
>>>>>>                    unexpected_keys = (diffusers.models.model_loading_utils
                        .load_model_dict_into_meta(model, state_dict,
                        device=param_device, dtype=torch_dtype,
 ...
 {[', '.join(unexpected_keys)]}"""
                    ...
                    force_hook = True
>>>>>>                    device_map = (diffusers.models.model_loading_utils.
                        _determine_device_map(model, device_map, max_memory,
                        torch_dtype))
...
>>>>>>                        accelerate.load_checkpoint_and_dispatch(model, 
                            ...
                            torch_dtype, force_hooks=force_hook, strict=True)
...
>>>>>>                            accelerate.load_checkpoint_and_dispatch(model, 
                                ...
                                index_file, device_map, max_memory=
                                max_memory, offload_folder=offload_folder,
                                offload_state_dict=offload_state_dict,
                                dtype=torch_dtype, force_hooks=force_hook,
                                strict=True)
...
>>>>>>                state_dict = (diffusers.models.model_loading_utils.
                ...
                (model, missing_keys, unexpected_keys, mismatched_keys,
                    error_msgs) = (cls._load_pretrained_model(model,
                    state_dict, model_file, pretrained_model_name_or_path,
                    ignore_mismatched_sizes=ignore_mismatched_sizes))
                loading_info = {'missing_keys': missing_keys,
                    'unexpected_keys': unexpected_keys, 'mismatched_keys':
                    mismatched_keys, 'error_msgs': error_msgs}
        if torch_dtype is not None and not isinstance(torch_dtype, paddle.dtype
            ):
            raise ValueError(
        ...
        def _find_mismatched_keys(state_dict, model_state_dict, loaded_keys,
            ...
            if ignore_mismatched_sizes:
                for checkpoint_key in loaded_keys:
                    ...
                    if model_key in model_state_dict and tuple(state_dict[
                        checkpoint_key].shape) != tuple(model_state_dict[
                        model_key].shape):
                        mismatched_keys.append((checkpoint_key, tuple(
                            state_dict[checkpoint_key].shape), tuple(
...
>>>>>>            error_msgs = (diffusers.models.model_loading_utils.
                _load_state_dict_into_model(model_to_load, state_dict))
        if len(error_msgs) > 0:
            error_msg = '\n\t'.join(error_msgs)
...
You should probably TRAIN this model on a down-stream task to be able to use it for predictions and inference."""
    ...
    def _get_signature_keys(cls, obj):
        ...
        required_parameters = {k: v for k, v in parameters.items() if v.
    ...
    def num_parameters(self, only_trainable: bool=False, exclude_embeddings:
        ...
        if exclude_embeddings:
            embedding_param_names = [f'{name}.weight' for name, module_type in
                self.named_sublayers(include_self=True) if isinstance(
                module_type, paddle.nn.Embedding)]
            non_embedding_parameters = [parameter for name, parameter in
                self.named_parameters() if name not in embedding_param_names]
            return sum(p.size for p in non_embedding_parameters if not p.
                stop_gradient or not only_trainable)
        else:
            return sum(p.size for p in self.parameters() if not p.
                stop_gradient or not only_trainable)

    def _convert_deprecated_attention_blocks(self, state_dict: OrderedDict
        ) ->None:
    ...
    def _undo_temp_convert_self_to_deprecated_attention_blocks(self) ->None:
        ...
        for module in deprecated_attention_block_modules:
            ...
            module.to_out = paddle.nn.LayerList(sublayers=[module.proj_attn,
                paddle.nn.Dropout(p=module.dropout)])
            del module.query
            del module.key
            del module.value
...
>>>>>>        user_agent = {'diffusers': diffusers.__version__, 'file_type':
            'model', 'framework': 'pytorch'}
        config, _, _ = cls.load_config(config_path, cache_dir=cache_dir,
            return_unused_kwargs=True, return_commit_hash=True,
            force_download=force_download, proxies=proxies,
            local_files_only=local_files_only, token=token, revision=
            revision, subfolder=subfolder, user_agent=user_agent, **kwargs)
>>>>>>        remapped_class = (diffusers.models.model_loading_utils.
            _fetch_remapped_cls_from_config(config, cls))
