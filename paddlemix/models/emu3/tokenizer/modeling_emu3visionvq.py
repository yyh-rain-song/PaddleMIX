...
class Emu3VisionVQCausalConv3d(paddle.nn.Layer):
    ...
    def __init__(self, in_channel: int, out_channel: int, kernel_size:
        Union[int, Tuple[int, ...]]=(3, 1, 1), stride: Union[int, Tuple[int,
        ...]]=(1, 1, 1)):
        super().__init__()
        ...
        self.padding += 2, 0
        self.conv = paddle.nn.Conv3D(in_channels=in_channel, out_channels=
            out_channel, kernel_size=kernel_size, stride=stride)

    def forward(self, x: paddle.Tensor):
        x = paddle.nn.functional.pad(x=x, pad=self.padding,
            pad_from_left_axis=False)
        x = self.conv(x)
        return x
...
class Emu3VisionVQResnetTemporalBlock(paddle.nn.Layer):
    ...
    def __init__(self, in_channels: int, out_channels: Optional[int]=None,
        ...
        self.conv1 = Emu3VisionVQCausalConv3d(in_channels, out_channels,
            kernel_size=kernel_size, stride=stride)
        self.norm2 = paddle.nn.BatchNorm3D(num_features=out_channels)
        self.dropout = paddle.nn.Dropout(p=dropout)
        self.conv2 = Emu3VisionVQCausalConv3d(out_channels, out_channels,
            kernel_size=kernel_size, stride=stride)
        self.act = Emu3VisionVQActivation()
        if self.in_channels != self.out_channels:
            if self.use_conv_shortcut:
                self.conv_shortcut = Emu3VisionVQCausalConv3d(in_channels,
                    out_channels, kernel_size=kernel_size, stride=stride)
            else:
                self.nin_shortcut = paddle.nn.Conv3D(in_channels=
...
class Emu3VisionVQSpatialNorm(paddle.nn.Layer):
    ...
    def __init__(self, f_channels: int, zq_channels: int, norm_layer:
        paddle.nn.Layer=paddle.nn.GroupNorm, add_conv: bool=False,
        num_groups: int=32, epsilon: float=1e-06):
        super().__init__()
        self.norm_layer = norm_layer(num_channels=f_channels, num_groups=
            num_groups, epsilon=epsilon)
        self.add_conv = add_conv
        if self.add_conv:
            self.conv = paddle.nn.Conv2D(in_channels=zq_channels,
                out_channels=zq_channels, kernel_size=3, stride=1, padding=1)
        self.conv_y = paddle.nn.Conv2D(in_channels=zq_channels,
            out_channels=f_channels, kernel_size=1, stride=1, padding=0)
        self.conv_b = paddle.nn.Conv2D(in_channels=zq_channels,
            out_channels=f_channels, kernel_size=1, stride=1, padding=0)

    def forward(self, x: paddle.Tensor, zq: paddle.Tensor):
        zq = paddle.nn.functional.interpolate(x=zq, size=tuple(x.shape)[-2:
            ], mode='nearest')
        if self.add_conv:
            zq = self.conv(zq)
        x = self.norm_layer(x)
...
class Emu3VisionVQResnetBlock(paddle.nn.Layer):
    ...
    def __init__(self, in_channels: int, out_channels: Optional[int]=None,
        conv_shortcut: bool=False, dropout: float=0.0, zq_ch: Optional[int]
        =None, add_conv: bool=False):
        super().__init__()
        self.in_channels = in_channels
        out_channels = in_channels if out_channels is None else out_channels
        self.out_channels = out_channels
        ...
        if zq_ch is None:
            ...
            self.norm1 = Emu3VisionVQSpatialNorm(in_channels, zq_ch,
                add_conv=add_conv)
            self.norm2 = Emu3VisionVQSpatialNorm(out_channels, zq_ch,
                add_conv=add_conv)
...
class Emu3VisionVQAttnBlock(paddle.nn.Layer):
    ...
    def __init__(self, in_channels: int, zq_ch: Optional[int]=None,
        add_conv: bool=False):
        super().__init__()
        self.in_channels = in_channels
        self.zq_ch = zq_ch
        if zq_ch is None:
            ...
            self.norm = paddle.nn.GroupNorm(num_channels=in_channels, **
                norm_kwargs)
        else:
            self.norm = Emu3VisionVQSpatialNorm(in_channels, zq_ch,
                add_conv=add_conv)
...
class Emu3VisionVQTemporalUpsample(paddle.nn.Layer):
    ...
    def __init__(self, in_channel: int, out_channel: int, kernel_size:
        Tuple[int, ...]=(3, 3, 3), stride: Tuple[int, ...]=(1, 1, 1)):
        super().__init__()
        self.in_channel = in_channel
        self.out_channel = out_channel
        self.conv = Emu3VisionVQCausalConv3d(in_channel, out_channel,
...
class Emu3VisionVQTemporalDownsample(paddle.nn.Layer):
    ...
    def __init__(self, in_channel: int, out_channel: int, kernel_size:
        Tuple[int, ...]=(4, 3, 3), stride: Tuple[int, ...]=(2, 1, 1)):
        super().__init__()
        self.in_channel = in_channel
        self.out_channel = out_channel
...
class Emu3VisionVQVectorQuantizer(paddle.nn.Layer):
    ...
    def forward(self, x: paddle.Tensor):
        ...
        d = paddle.sum(x=x_flattened ** 2, axis=1, keepdim=True) + paddle.sum(x
            =codebook ** 2, axis=1) - 2 * paddle.einsum('bd,dn->bn',
            x_flattened, codebook.transpose(perm=[1, 0]))
...
class Emu3VisionVQEncoder(paddle.nn.Layer):
    ...
    def __init__(self, config: Emu3VisionVQConfig):
        ...
        self.conv_in = paddle.nn.Conv2D(in_channels=self.in_channels,
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                block.append(Emu3VisionVQResnetBlock(in_channels=block_in,
                    out_channels=block_out, dropout=config.dropout))
                block_in = block_out
        ...
        self.mid.block_1 = Emu3VisionVQResnetBlock(in_channels=block_in,
            out_channels=block_in, dropout=config.dropout)
        self.mid.attn_1 = Emu3VisionVQAttnBlock(block_in)
        self.mid.block_2 = Emu3VisionVQResnetBlock(in_channels=block_in,
            out_channels=block_in, dropout=config.dropout)
        self.norm_out = paddle.nn.GroupNorm(num_channels=block_in,
            num_groups=32, epsilon=1e-06, weight_attr=True, bias_attr=True)
        out_z_channels = (2 * config.z_channels if config.double_z else
            config.z_channels)
        self.conv_out = paddle.nn.Conv2D(in_channels=block_in, out_channels
            =out_z_channels, kernel_size=3, stride=1, padding=1)
        ...
        for i in range(temporal_down_blocks):
            conv = Emu3VisionVQTemporalDownsample(out_z_channels,
                out_z_channels)
            self.time_conv.append(conv)
        self.time_res_stack = paddle.nn.Sequential(*[
            Emu3VisionVQResnetTemporalBlock(in_channels=out_z_channels,
            out_channels=out_z_channels, dropout=config.dropout) for _ in
            range(self.num_res_blocks)])
        self.act = Emu3VisionVQActivation()
...
class Emu3VisionVQDecoder(paddle.nn.Layer):
    ...
    def __init__(self, config: Emu3VisionVQConfig):
        ...
        self.time_res_stack = paddle.nn.Sequential(*[
            Emu3VisionVQResnetTemporalBlock(in_channels=config.z_channels,
            out_channels=config.z_channels, dropout=config.dropout) for _ in
            range(config.num_res_blocks)])
        tempo_upsample_block_num = int(math.log2(config.
            temporal_downsample_factor))
        self.time_conv = paddle.nn.LayerList()
        for i in range(tempo_upsample_block_num):
            conv = Emu3VisionVQTemporalUpsample(config.z_channels, config.
                z_channels)
            self.time_conv.append(conv)
        self.conv_in = paddle.nn.Conv2D(in_channels=config.z_channels,
            out_channels=block_in, kernel_size=3, stride=1, padding=1)
        self.mid = paddle.nn.Layer()
        self.mid.block_1 = Emu3VisionVQResnetBlock(in_channels=block_in,
            out_channels=block_in, dropout=config.dropout, zq_ch=zq_ch)
        ...
        self.conv_out = paddle.nn.Conv2D(in_channels=block_in, out_channels
            =config.out_channels, kernel_size=3, stride=1, padding=1)
...
class Emu3VisionVQModel(Emu3VisionVQPretrainedModel):
    ...
    def decode(self, x: paddle.Tensor):
        ...
        video = video.reshape([b, t * self.config.temporal_downsample_factor,
            self.config.out_channels, h * self.spatial_scale_factor, w *
